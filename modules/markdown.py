...
def parse_snip(m):
    try:
        name = m.group(1)[1:]
        snip = bot._plugins['snip'].storage.snips[name]
        if snip['type'] == bot._plugins['snip'].TYPE_TEXT:
...
async def reparse(event):
    ...
    message, msg_entities = await bot._parse_message_text(event.raw_text, parser)
    if len(old_entities) >= len(msg_entities) and event.raw_text == message:
        return

    await bot(EditMessageRequest(
