...
async def get_target_message(event):
    if event.is_reply and (await event.get_reply_message()).from_id == bot.uid:
...
async def await_read(chat, message):
    ...
    if await util.is_read(bot, chat, message):
        fut.cancel()
        return
...
@bot.on(util.admin_cmd(r"^\.(del)(?:ete)?$"))
@bot.on(util.admin_cmd(r"^\.(edit)(?:\s+(.*))?$"))
async def delete(event):
    ...
    if target:
        ...
        if command == 'edit':
            ...
            await bot.delete_messages(chat, target)
